...
Ractive.components['node'] = Ractive.extend({
	...
	computed: {
		default_class: function() {
			return this.container.get('node-class')
		}
	},
	template: "<div id='node_{{id}}' class='{{default_class}} {{class}}' style='position: absolute; z-index: 2; left: {{x}}px; top: {{y}}px; {{style}}' onclick='{{onclick}}'>{{yield}}</div>",
	onrender: function() {
		var self = this
		var div = self.find('*')
...
Ractive.components['edge'] = Ractive.extend({
	...
	computed: {
		default_class: function() {
			return this.container.get('edge-class')
		}
	},
	template: "<svg style='position: absolute; z-index: 1; display: inline-block; width: 100%; height: 100%; pointer-events: none;' ><line x1='{{x1}}' y1='{{y1}}' x2='{{x2}}' y2='{{y2}}' class='{{default_class}} {{class}}' style='pointer-events: auto; {{style}}' onclick='{{onclick}}' /></svg><span style='position: absolute; z-index: 2;'>{{yield}}</span>",
	oninit: function() {
